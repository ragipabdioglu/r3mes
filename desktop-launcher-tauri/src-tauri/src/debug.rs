...
fn collect_system_info() -> Result<SystemInfo, String> {
    ...
    let hostname = silent_command("hostname")
        .output()
    ...
    let uptime = if cfg!(unix) {
        silent_command("uptime")
            .arg("-s")
            .output()
            .ok()
            .and_then(|output| {
                let _uptime_str = String::from_utf8_lossy(&output.stdout);
...
fn get_disk_space() -> u64 {
    let _workspace = crate::platform::get_workspace_dir()
    ...
    #[cfg(windows)]
    {
        // On Windows, we could use GetDiskFreeSpaceExW
        // For now, return 0 as a placeholder
        // A proper implementation would use winapi crate
    }
...
async fn get_process_info(process_name: &str) -> Result<ProcessDebugInfo, String> {
    #[cfg(unix)]
    {
        let output = silent_command("pgrep")
            .arg("-f")
            .arg(process_name)
            .output()
...
fn get_process_details(_pid: u32) -> Result<(f64, u64, u64, String), String> {
    #[cfg(unix)]
    {
...
fn collect_config_info() -> Result<ConfigDebugInfo, String> {
    let workspace_path = crate::platform::get_workspace_dir()
        .unwrap_or_else(|_| PathBuf::from("."));
    let config_dir = crate::platform::get_config_dir()
        .unwrap_or_else(|_| PathBuf::from("."));
    
    let config_files = vec![
        config_dir.join("config.json"),
        config_dir.join("launcher_config.json"),
    ...
    for config_file in config_files {
        let modified_time = config_file.metadata()
            .ok()
...
pub async fn export_debug_info() -> Result<String, String> {
    ...
    let workspace_dir = crate::platform::get_workspace_dir()
        .unwrap_or_else(|_| PathBuf::from("."));
