...
fn check_cuda() -> CudaInfo {
    ...
    let runtime_version = Command::new("nvidia-smi")
        .output()
        .ok()
        .and_then(|output| {
...
fn check_disk_space() -> DiskInfo {
    ...
    // Required space: 50GB for models + 10GB for logs/cache
    let required_gb = 60.0;
