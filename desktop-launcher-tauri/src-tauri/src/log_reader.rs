...
impl LogReader {
    ...
    async fn start_file_checker(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for (_file_name, watcher) in watchers.iter_mut() {
                    if let Err(e) = Self::check_file_for_updates_simple(watcher).await {
    ...
    /// Check a file for new content (simplified version without parser)
    async fn check_file_for_updates_simple(
        watcher: &mut LogWatcher,
    ) -> Result<(), String> {
        let metadata = watcher.file.metadata()
        ...
        if current_size > watcher.position {
            ...
            let mut reader = BufReader::new(&watcher.file);
            reader.seek(SeekFrom::Start(watcher.position))
            ...
            for line in reader.lines() {
                if let Ok(line) = line {
                    ...
                    let entry = LogEntry {
                        ...
                        message: line.clone(),
                        raw_line: line,
                        line_number: watcher.line_number,
                        file_path: watcher.file_path.to_string_lossy().to_string(),
                    };
                    
                    let _ = watcher.sender.send(entry);
                }
            }
...
// Use lazy_static for regex caching to avoid recompilation on every parse

use once_cell::sync::Lazy;

// Cached regex patterns for better performance
static TENDERMINT_LOG_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^([IWEF])\[([^\]]+)\]\s+(.+)").expect("Invalid Tendermint regex")
});

static PYTHON_LOG_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2},\d{3}) - (\w+) - (.+)").expect("Invalid Python regex")
});

static IPFS_LOG_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^([^\t]+)\t(\w+)\t(.+)").expect("Invalid IPFS regex")
});
...
impl LogParser for TendermintLogParser {
    fn parse_line(&self, line: &str, source: &str, line_number: u64, file_path: &str) -> Option<LogEntry> {
        ...
        if let Some(captures) = TENDERMINT_LOG_REGEX.captures(line) {
            let level_char = captures.get(1).map(|m| m.as_str())?;
...
impl LogParser for PythonLogParser {
    fn parse_line(&self, line: &str, source: &str, line_number: u64, file_path: &str) -> Option<LogEntry> {
        ...
        if let Some(captures) = PYTHON_LOG_REGEX.captures(line) {
            let timestamp_str = captures.get(1).map(|m| m.as_str())?;
            let level_str = captures.get(2).map(|m| m.as_str())?;
            let message = captures.get(3).map(|m| m.as_str())?;
...
impl LogParser for IPFSLogParser {
    fn parse_line(&self, line: &str, source: &str, line_number: u64, file_path: &str) -> Option<LogEntry> {
        ...
        if let Some(captures) = IPFS_LOG_REGEX.captures(line) {
            let timestamp_str = captures.get(1).map(|m| m.as_str())?;
            let level_str = captures.get(2).map(|m| m.as_str())?;
            let message = captures.get(3).map(|m| m.as_str())?;
