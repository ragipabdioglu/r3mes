...
impl ModelDownloader {
    ...
    /// Get available models from blockchain
    pub async fn get_available_models(&self) -> Result<Vec<ModelInfo>, String> {
        // Query blockchain for approved models
        use crate::config::BlockchainConfig;
        let config = BlockchainConfig::from_network_mode(
            &std::env::var("NETWORK_MODE").unwrap_or_else(|_| "testnet".to_string())
        );
        
        // Use blockchain query to get approved models
        let query_url = format!("{}/cosmos/remes/model/approved_models", config.rest_endpoint);
                ...
                match response.json::<serde_json::Value>().await {
                    Ok(json) => {
                        let models = json["models"].as_array()
                            .ok_or("Invalid response format: missing models array")?;
                        
                        let mut model_infos = Vec::new();
                        for model in models {
                            let model_info = ModelInfo {
                                name: model["name"].as_str().unwrap_or("unknown").to_string(),
                                version: model["version"].as_str().unwrap_or("1.0.0").to_string(),
                                description: model["description"].as_str().unwrap_or("").to_string(),
                                size_bytes: model["size_bytes"].as_u64().unwrap_or(0),
                                checksum: model["checksum"].as_str().unwrap_or("").to_string(),
                                ipfs_hash: model["ipfs_hash"].as_str().unwrap_or("").to_string(),
                                download_urls: vec![
                                    format!("{}/ipfs/{}", self.ipfs_gateway, model["ipfs_hash"].as_str().unwrap_or(""))
                                ],
                                model_type: match model["model_type"].as_str().unwrap_or("custom") {
                                    "bitnet" => ModelType::BitNet,
                                ...
                                requirements: ModelRequirements {
                                    min_vram_gb: model["requirements"]["min_vram_gb"].as_f64().unwrap_or(8.0),
                                    min_ram_gb: model["requirements"]["min_ram_gb"].as_f64().unwrap_or(16.0),
                                    cuda_required: model["requirements"]["cuda_required"].as_bool().unwrap_or(true),
                                    python_version: model["requirements"]["python_version"].as_str().unwrap_or(">=3.8").to_string(),
                                    dependencies: model["requirements"]["dependencies"].as_array()
                    ...
                    Err(e) => Err(format!("Failed to parse blockchain response: {}", e))
                }
            }
            Ok(response) => Err(format!("Blockchain query failed with status: {}", response.status())),
            Err(e) => {
                // Fallback to hardcoded models if blockchain is unavailable
                println!("⚠️  Blockchain unavailable, using fallback models: {}", e);
                Ok(vec![
                    ModelInfo {
    ...
    pub async fn download_model(&self, model_info: &ModelInfo) -> Result<DownloadResult, String> {
        // SECURITY: Require valid checksum before download
        if model_info.checksum.is_empty() || model_info.checksum.len() < 32 {
            return Err("Model has no valid checksum - refusing to download for security reasons".to_string());
        }
        ...
        // Try different download sources with retry
        let mut download_success = false;
        let mut last_error = String::new();
        
        // Try IPFS first with retry
        if !model_info.ipfs_hash.is_empty() {
            for attempt in 0..self.max_retries {
                match self.download_from_ipfs(&model_info.ipfs_hash, &cache_file).await {
                    ...
                    Err(e) => {
                        last_error = format!("IPFS download failed (attempt {}): {}", attempt + 1, e);
                        println!("{}", last_error);
        ...
        // Try direct URLs if IPFS failed (with retry)
        if !download_success {
            for url in &model_info.download_urls {
                for attempt in 0..self.max_retries {
                    match self.download_from_url(url, &cache_file, model_info.size_bytes).await {
                        ...
                        Err(e) => {
                            last_error = format!("URL download failed (attempt {}): {}", attempt + 1, e);
                            println!("{}", last_error);
                            if attempt < self.max_retries - 1 {
                                let delay = self.retry_delay_ms * (2_u64.pow(attempt));
                                tokio::time::sleep(std::time::Duration::from_millis(delay)).await;
        ...
        if !download_success {
            return Err(format!("All download attempts failed after {} retries. Last error: {}", self.max_retries, last_error));
        }
        
        // MANDATORY: Verify checksum
        ...
        if actual_checksum != model_info.checksum {
            ...
            return Err(format!(
                "SECURITY ERROR: Checksum verification failed. Expected: {}, Got: {}. Model may be corrupted or tampered.",
        ...
        Ok(DownloadResult {
            ...
            message: format!("Model {} downloaded and verified successfully", model_info.name),
            installation_path: Some(model_dir.to_string_lossy().to_string()),
    ...
    pub async fn get_download_progress(&self, _model_name: &str) -> Option<DownloadProgress> {
    ...
    pub async fn cancel_download(&self, _model_name: &str) -> Result<(), String> {
