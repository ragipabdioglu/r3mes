//! Setup checker for R3MES Desktop Launcher
...
use log::{info, warn};
use crate::hardware_check::check_hardware;
...
impl SetupChecker {
    ...
    pub async fn check_complete_setup() -> Result<SetupStatus, String> {
        ...
        // Check system requirements using hardware_check
        let hw_check = check_hardware();
        
        status.docker_installed = hw_check.docker.installed && hw_check.docker.running;
        status.cuda_available = hw_check.cuda.installed;
    ...
    async fn check_engine_installation() -> bool {
        ...
        let engine_paths: Vec<PathBuf> = vec![
            PathBuf::from("miner-engine"),
            PathBuf::from("../miner-engine"),
            PathBuf::from("../../miner-engine"),
        ];
        
        // Add home directory path
        let mut all_paths = engine_paths;
        if let Some(home) = dirs::home_dir() {
            all_paths.push(home.join("r3mes").join("miner-engine"));
        }
    ...
    async fn check_models_installation() -> bool {
        ...
        let model_paths: Vec<PathBuf> = vec![
            PathBuf::from("models"),
            PathBuf::from("../models"), 
            PathBuf::from("../../models"),
        ];
        ...
        // Add home directory path
        let mut all_paths = model_paths;
        if let Some(home) = dirs::home_dir() {
            all_paths.push(home.join("r3mes").join("models"));
        }
    ...
    async fn check_wallet_configuration() -> bool {
        // Use platform module for cross-platform path
        let wallet_path = match crate::platform::get_wallets_dir() {
            Ok(dir) => dir.join("default_wallet.json"),
            Err(_) => return false,
        };
    ...
    pub async fn validate_component(component: &str) -> Result<bool, String> {
        ...
        match component {
            "docker" => Ok(hw_check.docker.installed && hw_check.docker.running),
            "python" => Ok(Self::check_python_installation().await),
            "git" => Ok(Self::check_git_installation().await),
            "cuda" => Ok(hw_check.cuda.installed),
...
// Note: Tauri commands are defined in commands.rs to avoid duplication
// These functions are called from commands.rs