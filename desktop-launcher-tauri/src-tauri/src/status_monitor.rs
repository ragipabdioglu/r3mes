...
impl StatusMonitor {
    ...
    /// Get disk space information
    async fn get_disk_space(_path: &std::path::Path) -> (u64, u64) {
        #[cfg(unix)]
        {
    ...
    async fn get_process_status_by_name(_process_name: &str) -> Option<ProcessStatus> {
        #[cfg(unix)]
        {
    ...
    async fn check_service_status(name: &str, url: &str) -> ServiceStatus {
        ...
        // Extract port from URL - simple parsing without url crate
        let port = if let Some(port_start) = url.rfind(':') {
            let port_str = &url[port_start + 1..];
            // Find where port ends (at / or end of string)
            let port_end = port_str.find('/').unwrap_or(port_str.len());
            port_str[..port_end].parse::<u16>().ok()
