...
from utils.verification import VerificationPolicy, VerificationLevel
...
class ServingEngine:
    ...
    def __init__(
        ...
        # Unified Registry for secure model/adapter loading
    ...
    async def load_model(self, model_ipfs_hash: Optional[str] = None) -> bool:
        ...
        Uses UnifiedRegistry for secure, verified model loading.
        ...
        try:
            ...
            self.logger.info(f"Loading model with verification (hash: {hash_to_load})")
            
            # Use UnifiedRegistry for secure model loading
            result = self._registry.ensure_ready_atomic(
                required_adapters=["general_dora"],
                require_dataset=False,  # Dataset not required for serving
            )
            
            if not result.success:
                self.logger.error(f"Model loading failed: {result.error_message}")
                self._health.error_message = result.error_message
                self._health.state = EngineState.ERROR
                return False
            
            model_path = result.status.model_path
            if not model_path:
                self.logger.error("No model path returned from registry")
                return False
            
            self.logger.info(f"Model verified and ready at: {model_path}")
            ...
            if success:
                self.model_ipfs_hash = hash_to_load or result.status.model_version
                self._health.model_loaded = True
                self._health.state = EngineState.READY
                self._health.is_ready = True
                self.logger.info(f"Model loaded and verified successfully")
                return True
            else:
