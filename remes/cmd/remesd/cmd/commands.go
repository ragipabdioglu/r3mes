...
func initRootCmd(
	...
	server.AddCommandsWithStartCmdOptions(rootCmd, app.DefaultNodeHome, newApp, appExport, server.StartCmdOptions{
		...
		// PostSetup hook is called after server setup, allowing us to verify TLS configuration
		PostSetup: func(svrCtx *server.Context, clientCtx client.Context, ctx context.Context, g *errgroup.Group) error {
			return setupTLSForGRPCServer(svrCtx.Logger, svrCtx.Config.RootDir)
		},
		PostSetupStandalone: func(svrCtx *server.Context, clientCtx client.Context, ctx context.Context, g *errgroup.Group) error {
			return setupTLSForGRPCServer(svrCtx.Logger, svrCtx.Config.RootDir)
		},
		// Note: Cosmos SDK v0.50.x doesn't directly support GRPCServerOptions in StartCmdOptions
		// TLS configuration is handled via environment variables and automatic detection
		// See tls_server.go for implementation details
	})
...
func queryCommand() *cobra.Command {
	...
	cmd.AddCommand(
		...
		GetRemesQueryCmd(), // Add custom remes module query commands
	)
