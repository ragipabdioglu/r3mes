...
type TrainingKeeper struct {
	...
	economics *economics.EconomicsKeeper // KRİTİK: Trap job slash/reward için gerekli
	...
	TrapJobs                      collections.Map[uint64, types.TrapJob] // Trap job storage
}
...
func NewTrainingKeeper(
	...
	k := &TrainingKeeper{
		...
		TrapJobs:                      collections.NewMap(sb, types.TrapJobKey, "trap_jobs", collections.Uint64Key, codec.CollValue[types.TrapJob](cdc)),
	}
...
// SetEconomicsKeeper sets the economics keeper reference
// This is called after both keepers are initialized to avoid circular dependency
func (k *TrainingKeeper) SetEconomicsKeeper(economicsKeeper *economics.EconomicsKeeper) {
...
func (k *TrainingKeeper) SubmitGradient(ctx context.Context, gradient types.StoredGradient) error {
	...
	// KRİTİK: Trap Job Verification
	// Check if this gradient is for a trap job and verify the hash
	trapResult, err := k.VerifyTrapJobResult(ctx, gradient)
	if err != nil {
		return fmt.Errorf("failed to verify trap job: %w", err)
	}

	// Process trap job verdict (slash/reward)
	if trapResult.Verdict != VerdictNormalJob {
		if err := k.ProcessTrapJobVerdict(ctx, trapResult); err != nil {
			return fmt.Errorf("failed to process trap job verdict: %w", err)
		}

		// If trap job failed, reject the gradient
		if trapResult.Verdict == VerdictTrapFailed || trapResult.Verdict == VerdictTimeout {
			return fmt.Errorf("trap job verification failed: %s", trapResult.ErrorMessage)
		}
	}
